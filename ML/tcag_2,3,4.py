...
x_train, x_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=12) #8
...
risk_scores_df.to_excel("out_data/tacg_risk_scores.xlsx")
...
gse_train_roc_df.to_excel(f"out_data/tacg_train_roc_auc({train_auc}).xlsx")
gse_test_roc_df = pd.DataFrame({
    "fpr": fpr,
    "tpr": tpr,
})
gse_test_roc_df.to_excel(f"out_data/tacg_test_roc_auc({auc}).xlsx")
